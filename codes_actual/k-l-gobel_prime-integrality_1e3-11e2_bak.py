...
pdf = PdfPages('k-l-gobel_prime-integrality_1e3-11e2.pdf')
integrality_list = []

for p in primerange(1000, 1100):
    print("-------- Set p =", p, "--------", flush = True)
    res_mat_ary = np.zeros((p, p), dtype = int) - 1
    ...
    pdf.savefig()
    plt.close()
    with open('k-l-gobel_prime-integrality_1e3-11e2.pkl', 'wb') as handle:
