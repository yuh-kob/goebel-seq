...
for p in primerange(3000, 100000):
  if p % 4 == 1:
    print("-------- Set p =", p, "--------", flush = True)
    res_vec_ary = np.zeros(p, dtype = int) - 1
    for k in range((p - 1)//2, (p - 1)//2 + 1):
      for l in range(0, p):
        m_max = p
        g_1 = l  # (1 + 1^2)/1
        m = 1
        ...
        if g_m_state is None:
            ...
            res_vec_ary[l] = 0  # Modified at latest
            # break
        else:
            # print("Pass for k =", k)
            res_vec_ary[l] = 1  # Modified at latest
    # print(res_vec_ary)
    integrality_list.append(res_vec_ary)
    ...
    # pdf.savefig()
    plt.close()'''
    with open('k-l-gobel-mid_prime-integrality_3e3-1e5.pkl', 'wb') as handle:
