...
if len(sys.argv) < 3:
    print("Usage: python3 this.py range_low range_up", file = sys.stderr)
    exit()

range_low = int(sys.argv[1])
range_up = int(sys.argv[2])
print("Command: python3 this.py %d %d" % (range_low, range_up), file = sys.stderr)

factorint_vec = np.vectorize(factorint)
factor_ary = factorint_vec(np.arange(0, 20000))
...
for k in range(range_low, range_up):
